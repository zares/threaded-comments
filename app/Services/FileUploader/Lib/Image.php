...
namespace App\Services\FileUploader\Lib;

/**
 *  Methods used with the {@link resize()} method.
 */

define('IMAGE_BOXED', 0);
define('IMAGE_NOT_BOXED', 1);
define('IMAGE_CROP_TOPLEFT', 2);
define('IMAGE_CROP_TOPCENTER', 3);
define('IMAGE_CROP_TOPRIGHT', 4);
define('IMAGE_CROP_MIDDLELEFT', 5);
define('IMAGE_CROP_CENTER', 6);
define('IMAGE_CROP_MIDDLERIGHT', 7);
define('IMAGE_CROP_BOTTOMLEFT', 8);
define('IMAGE_CROP_BOTTOMCENTER', 9);
define('IMAGE_CROP_BOTTOMRIGHT', 10);
...
class Image {
     ...
     *  $img->resize(150, 150, IMAGE_CROP_CENTER);
     *  </code>
     *
     ...
     *
     *                                          -   **IMAGE_BOXED** - the image will be scaled so that it will fit
     ...
     *
     *                                          -   **IMAGE_NOT_BOXED** - the image will be scaled so that it
     ...
     *
     *                                          -   **IMAGE_CROP_TOPLEFT**
     *                                          -   **IMAGE_CROP_TOPCENTER**
     *                                          -   **IMAGE_CROP_TOPRIGHT**
     *                                          -   **IMAGE_CROP_MIDDLELEFT**
     *                                          -   **IMAGE_CROP_CENTER**
     *                                          -   **IMAGE_CROP_MIDDLERIGHT**
     *                                          -   **IMAGE_CROP_BOTTOMLEFT**
     *                                          -   **IMAGE_CROP_BOTTOMCENTER**
     *                                          -   **IMAGE_CROP_BOTTOMRIGHT**
     *
     ...
     *                                          wrong.
     */
    public function resize($width = 0, $height = 0, $method = IMAGE_CROP_CENTER, $background_color = -1) {
        ...
        if ($this->_create_from_source()) {
            ...
            if ($this->preserve_aspect_ratio || isset($auto_preserve_aspect_ratio)) {
                ...
                // if both width and height are given and IMAGE_BOXED or IMAGE_NOT_BOXED methods are to be used
            ...
            if (
                ...
                // it needs to be preserved and method is IMAGE_BOXED or IMAGE_NOT_BOXED
                } else {

                    // prepare the target image
                    $target_identifier = $this->_prepare_image(
                        ($width > 0 && $height > 0 && $method !== IMAGE_NOT_BOXED ? $width : $target_width),
                        ($width > 0 && $height > 0 && $method !== IMAGE_NOT_BOXED ? $height : $target_height),
                        $background_color
                    );
                    ...
                    imagecopyresampled(
                        $target_identifier,
                        $this->source_identifier,
                        ($width > 0 && $height > 0 && $method !== IMAGE_NOT_BOXED ? ($width - $target_width) / 2 : 0),
                        ($width > 0 && $height > 0 && $method !== IMAGE_NOT_BOXED ? ($height - $target_height) / 2 : 0),
                        0,
                        0,
                        $target_width,
                        $target_height,
