...
<form class="mb-6" id="comment-form" data-action="form#submit" data-form-action-param="{{ route('comment.'. $action) }}">
    ...
    <div class="flex text-left">
        <button type="submit" class="inline-flex items-center py-2.5 px-4 text-sm font-semibold text-center text-white bg-primary-700 rounded-lg focus:ring-4 focus:ring-primary-200" data-form-target="submit">
        ...
        <button data-action="form#selectImage" type="button" class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm pr-4 pl-3 py-2 text-center inline-flex items-center ml-2 mr-2">
            ...
            <span data-form-target="ispan">{{ ($comment->extra['old_image'] ?? null) ?: 'Upload Image' }}</span>
            <input data-form-target="image" data-action="change->form#imageName" type="file" name="image" accept="image/jpeg, image/jpg, image/gif, image/png" class="hidden">
        </button>
        <button data-action="form#selectFile" type="button" class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm pr-4 pl-3 py-2.5 text-center inline-flex items-center">
            ...
            <span data-form-target="fspan">{{ ($comment->extra['old_file'] ?? null) ?: 'Attach File' }}</span>
            <input data-form-target="file" data-action="change->form#fileName" type="file" name="file" accept="text/plain" class="hidden">
        </button>
    </div>
    <div class="text-right">
        <button class="text-sm text-blue-700 underline mr-1" type="button" data-action="index#toList">
            Back to list
        </button>
    </div>
</form>
